...
const createOrder = async (req, res) => {
  try {
    ...
    // Create the order without PayPal payment method
    const newlyCreatedOrder = new Order({
      userId,
      cartId,
      cartItems,
    ...
    // Remove the cart after order creation (since it's linked)
...
const capturePayment = async (req, res) => {
  try {
    ...
    // Reduce product stock based on the order's cart items
    for (let item of order.cartItems) {
      let product = await Product.findById(item.productId);
